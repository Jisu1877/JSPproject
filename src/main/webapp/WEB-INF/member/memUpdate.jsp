...
<head>
	...
	//우편번호 검색 다음 api
		...
		function previewDelete() {
			...
			//이미지를 삭제했다
			document.getElementById("noimageCheck").value = "noimage";
		...
		function previewImage(targetObj, previewId) { 
			...
			let uExt = ext.toUpperCase(); //확장자를 대문자로 변환
	 		...
	 		//이미지를 변경했다.
	 		document.getElementById("noimageCheck").value = "yesimage";
			...
			$("#demo").append("<div id='demoChild' style='margin-top: 5px;'>"
			+"<input type='button' id='photoDelete' value='삭제' class='w3-button w3-black w3-border w3-padding-small w3-tiny' onclick='previewDelete()'/>"	  
			+	"</div>");
		...
		<div class="w3-third w3-margin-bottom">
		    <form name="myForm" method="post" action="${ctp}/memUpdateOk.mem" class="was-validated" enctype="multipart/form-data">
		    	<div id="profile" style="text-align:center">
	    			...
	    			<div id="noimageID" style="text-align:center; display: none">
    					<img id="noimage" class="noimage" src="images/noimage.jpg" width="200px"/>
	    			</div>
	    			<div id='previewId' style="text-align:center;">
		    		</div>
    				<div id="originalPhoto">
			    		<c:if test="${!empty vo.idx}">
			    			<img id="image" src="${ctp}/data/member/${vo.save_file_name}" width="200px"/>
			    		</c:if>
		    		</div>
		    		<c:if test="${vo.save_file_name != 'noimage.jpg'}">
