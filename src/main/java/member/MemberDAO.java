...
public class MemberDAO {
	...
	//멤버 체크(로그인 체크)
		...
		public MemberVO getMemInfor(String name, String tel) {  //getMemInfor 메소드 오버로딩
			MemberVO vo = new MemberVO();
			try {
				sql = "select * from member where name = ? and tel = ? and del_yn = 'n'";
				pstmt = conn.prepareStatement(sql);
		...
		public MemberVO getMemInfor(String mid, String name, String tel) {  //getMemInfor 메소드 오버로딩
			MemberVO vo = new MemberVO();
			try {
				sql = "select * from member where mid = ? and name = ? and tel = ? and del_yn = 'n'";
				pstmt = conn.prepareStatement(sql);
				pstmt.setString(1, mid);
		...
		//예약시 사용포인트 차감
		public int setPoint(int mem_idx, int usePoint) {
			int res2 = 0;
			try {
